...
import { startDebitTransaction, startTransactionAsociateUser } from "../../../store/user/thunks";
...
export const ButtonTransfer = ({product_data}) => {
  const dispatch = useDispatch()
  const { user } = useSelector((state) => state.user_state);
  const [modal, setModal] = useState(false);
  const { amount, transactional_key, product_receive, onInputChange, onCleanForm } = useForm(initialState)
          ...
          <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                Transfer to Other Account Since:{" "}
